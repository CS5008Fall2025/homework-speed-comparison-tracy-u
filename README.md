[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/MC3_PSCr)
# Homework: Speed Comparison Between Vector, List, and Binary Search Tree

Name: Tracy U.

Github Account name: tracy-u

Link to Assignment on Github: 
- https://github.com/CS5008Fall2025/homework-speed-comparison-tracy-u
- `assignment` branch

How many hours did it take you to complete this assignment (estimate)? 

Did you collaborate with any other students/TAs/Professors? If so, tell us who and in what capacity.  
- I collaborated with my group for the Lab and referenced that work where relevant. 


Did you use any external resources (you do not have to cite in class material)? (Cite them below)  
- Those cited in the report. 
- https://www.geeksforgeeks.org/dsa/deletion-in-linked-list/
